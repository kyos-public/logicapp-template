...
- **SMS service provider** (Twilio, Azure Communication Services, or similar)
- **Required API permissions** granted in the target tenant:
  - `User.Read.All` (Microsoft Graph)
  - SMS service provider API access
...
3. **SMS Provider Setup**: Configure SMS service provider credentials in the target tenant
4. **Grant Admin Consent**: Some Graph API permissions require admin consent in the target tenant
...
This quickstart template creates a basic workflow to send pw by SMS after joiner workflow.
...
4. Configure SMS service provider connections in the Logic App designer
5. Test the workflow to ensure all connections are working properly
...
- **SMS Service Errors**: Verify SMS provider credentials and configuration
