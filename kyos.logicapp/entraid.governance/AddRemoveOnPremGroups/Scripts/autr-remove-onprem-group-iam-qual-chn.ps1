<#
.SYNOPSIS
    Removes a user from Active Directory groups based on access package mapping.

.DESCRIPTION
    This runbook removes a user from groups mapped to an access package. It supports two types of groups:
    1. Parent Group: Uses AD group nesting - user is removed from parent, loses sub-group access
    2. Additional Groups: Systems incompatible with nesting - user is removed directly from each group

.PARAMETER UserPrincipalName
    The User Principal Name (UPN) of the user to remove from groups (e.g., user@domain.com).
...
.PARAMETER MaxRetries
    Maximum number of retry attempts when removing a user from a group. Default is 3.
...
.EXAMPLE
    ...
    # Run the script
    .\Remove-UserFromDepartmentGroups.ps1 `
...
.NOTES
    ...
    User is removed from all groups in the list.
#>
...
function Remove-UserFromGroupWithRetry {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
    ...
    while ($attempt -lt $MaxAttempts) {
        ...
        try {
            ...
            Write-LogMessage "Successfully removed user from group '$($Group.Name)' (attempt $attempt)" -Level Success
            return $true
        }
        catch {
            $lastError = $_
    ...
    Write-LogMessage "Failed to remove user from group '$($Group.Name)' after $MaxAttempts attempts: $lastError" -Level Error
    return $false
}
...
try {
    Write-LogMessage "========================================"
    Write-LogMessage "Starting Access Package Removal"
    Write-LogMessage "========================================"
    ...
    $requiredCmdlets = @('Get-ADUser', 'Get-ADGroup', 'Get-ADGroupMember', 'Remove-ADGroupMember')
...
Write-LogMessage "Distinguished Name: $($user.DistinguishedName)"
    ...
    foreach ($groupName in $groupNames) {
        ...
        try {
            ...
            if (-not $isMember) {
                Write-LogMessage "User is not a member of '$groupName' - Skipping"
                $notMemberCount++
            }
            else {
                # Remove user from group
                $result = Remove-UserFromGroupWithRetry -User $user -Group $group -MaxAttempts $MaxRetries -DelaySeconds $RetryDelaySeconds
    ...
    Write-LogMessage "  Successfully Removed: $successCount"
    Write-LogMessage "  Not a Member: $notMemberCount"
